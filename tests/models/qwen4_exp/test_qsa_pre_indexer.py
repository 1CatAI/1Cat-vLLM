...
def _gemma_rmsnorm(
    ...
    """Reference Gemma RMSNorm that also runs on SM70."""
    input_float = input_.float()
    normalized = input_float * torch.rsqrt(
        input_float.square().mean(dim=-1, keepdim=True) + eps
    )
    return (normalized * (1.0 + weight.float())).to(input_.dtype)
...
def test_qsa_fused_pre_indexer_matches_unfused(
    ...
    compressed_rows = _gemma_rmsnorm(pooled.reshape(-1, D), k_weight, EPS).reshape(
        -1, 1, D
    )
    group_positions = (
