...
def test_initializes_and_builds_current_qwen_cache_topology(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    for group in proposer.draft_attn_groups:
        ...
        def build_for_drafting(*, common_attn_metadata, draft_index, group=group):
...
def test_qwen_proposer_hidden_and_config_contract() -> None:
    proposer = Qwen4ExpMTPProposer.__new__(Qwen4ExpMTPProposer)
    proposer.draft_model_config = SimpleNamespace(
