...
def test_video_patch_count_and_video_only_token_count(video_processor):
    out = run_video_preprocess(video_processor, torch.rand(7, 3, 300, 400))
    expected_patches = math.prod(out["video_grid_thw"][0].tolist())
    assert video_processor.get_number_of_video_patches(7, 300, 400) == expected_patches

    processor = object.__new__(Glm5NextProcessor)
    processor.video_processor = video_processor
    token_counts = processor._get_num_multimodal_tokens(video_sizes=[(7, 300, 400)])
    assert token_counts["num_video_tokens"] == [expected_patches // MERGE_SIZE**2]
