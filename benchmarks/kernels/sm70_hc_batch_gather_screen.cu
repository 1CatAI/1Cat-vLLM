...
// Research adaptation of PR #504 (5a049230cda4), no opaque communicator ABI.
// Raw IPC buffers are allocated and initialized by a dedicated owned channel.
...
void run(const std::vector<int64_t>& pointers, int rank, torch::Tensor input,
         torch::Tensor aux, torch::Tensor output) {
  ...
  TORCH_CHECK(rows >= 2 && rows <= 16 && aux.size(0) == rows &&
              output.size(0) == rows);
  ...
  gather_kernel<Mix><<<(packs + threads - 1) / threads, threads, 0, stream>>>(
      buffers, reinterpret_cast<const half*>(input.data_ptr()),
      Mix ? reinterpret_cast<const half*>(aux.data_ptr()) : nullptr,
      reinterpret_cast<half*>(output.data_ptr()),
      Mix ? nullptr : reinterpret_cast<half*>(aux.data_ptr()), rank, rows);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
}  // namespace sm70_hc_batch

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("down", &sm70_hc_batch::run<false>);
  m.def("mix", &sm70_hc_batch::run<true>);
}
