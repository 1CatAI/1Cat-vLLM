...
// Benchmark only: interleaved gate/up pairs fit in a single physical N32 tile.
// Split the original two-tile CTA without changing any accumulation sequence.
#define _C _C_moe_single_tile_reference
#include "../../csrc/sm70_turbomind/ops/nvfp4_grouped_decode_sm70.cu"
#undef _C
namespace {
template <int Split>
__global__
#ifdef MOE_SINGLE_TILE_BOUND
__launch_bounds__(32 * Split, Split == 8 ? 6 : 1)
#endif
    void single_tile_w13_kernel(const half* x, const uint32_t* weights,
  ...
  __shared__ float partial[Split][kPack][32];
  __shared__ half projected[kPack][32];
...
#pragma unroll
  for (int i = 0; i < 8; ++i) {
    ...
    partial[split][r][quad * 8 + c] = accum[i];
  }
  __syncthreads();
  for (int idx = threadIdx.x; idx < kPack * 32; idx += blockDim.x) {
    ...
    float value = 0;
#pragma unroll
    for (int s = 0; s < Split; ++s) value += partial[s][r][c];
    projected[r][c] = __float2half_rn(value);
  }
  __syncthreads();
  for (int idx = threadIdx.x; idx < count * 16; idx += blockDim.x) {
    const int r = idx / 16, c = idx % 16;
    const half gate = projected[r][2 * c];
    const half up = projected[r][2 * c + 1];
    ...
    out[static_cast<size_t>(rows[group_id * kPack + r]) * 160 +
        blockIdx.x * 16 + c] = __hmul(activated, up);
  }
}

void single_tile_w13(torch::Tensor out, torch::Tensor x, torch::Tensor w,
  ...
  TORCH_CHECK(interleaved,
              "Single-tile benchmark requires interleaved gate/up");
...
#define LAUNCH(S, I)                                                         \
  single_tile_w13_kernel<S><<<dim3(10, routes), 32 * S, 0, stream>>>(        \
...
std::vector<int64_t> resource_info() {
  ...
  AT_CUDA_CHECK(cudaFuncGetAttributes(&attr, single_tile_w13_kernel<S>));
  int blocks = 0;
  AT_CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &blocks, single_tile_w13_kernel<S>, 32 * S, 0));
...
TORCH_LIBRARY_FRAGMENT(_C_moe_single_tile, m) {
  m.def(
...
TORCH_LIBRARY_IMPL(_C_moe_single_tile, CUDA, m) {
  m.impl("run", &single_tile_w13);
}
