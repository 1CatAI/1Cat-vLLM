...
// Benchmark-only: include in the SAME DSO that owns CustomAllreduce lifecycle
// and methods. Never reinterpret a communicator pointer from another DSO.
#include "../../csrc/custom_all_reduce.cuh"
#include <c10/cuda/CUDAGuard.h>
#include <c10/cuda/CUDAStream.h>
#include <torch/all.h>

namespace hc_push_screen {
...
TORCH_LIBRARY_FRAGMENT(_C_custom_ar_flashnext, m) {
  m.def(
      ...
      "-> ()");
  m.impl("hc_down_gather", torch::kCUDA, &hc_push_screen::run<false>);
  m.def("hc_mix_gather(int ptr, Tensor gate, Tensor x, Tensor! output) -> ()");
  m.impl("hc_mix_gather", torch::kCUDA, &hc_push_screen::run<true>);
}
