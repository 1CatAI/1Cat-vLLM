...
// Benchmark-only scale tile-major locality; arithmetic/weights unchanged.
...
#pragma unroll 4
    for (int g = split * (160 / Split); g < (split + 1) * (160 / Split); ++g) {
      ...
      const half scalar = __hmul(__ldg(s + (tile * 160 + g) * 32 + col),
...
#pragma unroll
    for (int g = 0; g < 10; ++g) {
      ...
      const half scalar = __hmul(__ldg(s + (blockIdx.x * 10 + g) * 32 + col),
                                 __float2half_rn(16384.0f));
      const half2 scale = __halves2half2(scalar, scalar);
      half2 decoded[8];
      decode(__ldcs(w + offset), scale, decoded);
...
TORCH_LIBRARY_FRAGMENT(_C_moe_scale_layout, m) {
  m.def(
...
TORCH_LIBRARY_IMPL(_C_moe_scale_layout, CUDA, m) {
