...
// Benchmark-only compact task scheduling; not selected by serving or CMake.
// Arithmetic and grouping copied from nvfp4_grouped_decode_sm70.cu at
// 3e0f7a40c1. Both screened schedules were negative; retain as reproducible
// evidence, without modifying the existing production grouped-MoE kernel.
...
#ifndef SM70_MOE_COMPACT_WAVES
  #define SM70_MOE_COMPACT_WAVES 0
#endif
constexpr bool kCompactTasks = SM70_MOE_COMPACT_WAVES > 0;
...
__global__ void w13_kernel(const half* x, const uint32_t* weights,
  ...
  if constexpr (!kCompactTasks) {
    if (blockIdx.y >= *total) return;
  }
  const int task_begin = kCompactTasks ? blockIdx.x : 0;
  const int task_stride = kCompactTasks ? gridDim.x : 1;
  const int task_end = kCompactTasks ? *total * 5 : 1;
  for (int task = task_begin; task < task_end; task += task_stride) {
    const int group_id = kCompactTasks ? task / 5 : blockIdx.y;
    const int tile_x = kCompactTasks ? task % 5 : blockIdx.x;
    const int count = sizes[group_id], expert = experts[group_id];
    const int lane = threadIdx.x % 32, warp = threadIdx.x / 32;
    const int projection = warp / Split, split = warp % Split;
    const int tile =
        Interleaved ? tile_x * 2 + projection : tile_x + projection * 5;
...
#pragma unroll 4
      for (int g = split * (160 / Split); g < (split + 1) * (160 / Split);
           ++g) {
        const size_t offset =
...
#pragma unroll
    ...
    for (int idx = threadIdx.x; idx < count * 32; idx += blockDim.x) {
      ...
      out[static_cast<size_t>(rows[group_id * kPack + r]) * 160 + tile_x * 32 +
          c] = __hmul(activated, up);
    }
    // A CTA may now consume another valid group/tile. Finish every output read
    // before its peers reuse shared projection scratch. The existing numerical
    // reduction tree and original-route scatter are unchanged.
    if constexpr (kCompactTasks) __syncthreads();
  }
}
...
void run(torch::Tensor out, torch::Tensor x, torch::Tensor w, torch::Tensor s,
  ...
  const dim3 grid =
      kCompactTasks
          ? dim3(at::cuda::getCurrentDeviceProperties()->multiProcessorCount *
                 SM70_MOE_COMPACT_WAVES)
          : dim3(5, routes);
#define LAUNCH(S, I)                                                         \
  w13_kernel<S, I><<<grid, 64 * S, 0, stream>>>(                             \
...
__global__ void w2_kernel(const half* x, const uint32_t* weights,
                          ...
                          const int32_t* total, half* out) {
  if constexpr (!kCompactTasks) {
    if (blockIdx.y * 4 + threadIdx.x / 32 >= *total) return;
  }
  // Keep the original CTA's four distinct expert groups and common N tile.
  // Group-major warp tasks would also change weight/cache locality, confounding
  // this scheduling-only experiment with the previously screened warp layout.
  const int begin = kCompactTasks ? blockIdx.x : 0;
  const int stride = kCompactTasks ? gridDim.x : 1;
  const int end = kCompactTasks ? ((*total + 3) / 4) * 80 : 1;
  for (int task = begin; task < end; task += stride) {
    const int group = kCompactTasks ? task / 80 * 4 + threadIdx.x / 32
                                    : blockIdx.y * 4 + threadIdx.x / 32;
    if constexpr (kCompactTasks) {
      if (group >= *total) continue;
    }
    const int tile_x = kCompactTasks ? task % 80 : blockIdx.x;
...
#pragma unroll
      for (int g = 0; g < 10; ++g) {
        const int offset = (tile_x * 20 + g * 2) * 32 + col;
        const half scalar = __hmul(__ldg(s + (g * 80 + tile_x) * 32 + col),
                                   __float2half_rn(16384.0f));
        const half2 scale = __halves2half2(scalar, scalar);
        half2 decoded[8];
        decode(__ldcs(w + offset), scale, decoded);
...
#pragma unroll
    for (int i = 0; i < 8; ++i) {
      ...
      if (row < count) {
        ...
        out[static_cast<size_t>(dst) * 2560 + tile_x * 32 + quad * 8 + c] =
...
TORCH_LIBRARY_FRAGMENT(_C_moe_compact_tasks, m) {
  m.def(
...
TORCH_LIBRARY_IMPL(_C_moe_compact_tasks, CUDA, m) {
