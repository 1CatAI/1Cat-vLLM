...
#pragma unroll
  for (int k_offset = 0; k_offset < kD; k_offset += kPanelK) {
    load_swizzled_matrix_a_fragment(top_a_fragment, shared_query_top, k_offset);
...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    thread_max =
        fmaxf(thread_max, __shfl_down_sync(kFullWarpMask, thread_max, offset));
  }
...
__device__ __forceinline__ void scale_candidate_accumulators(
  ...
  scale_accumulator_two_rows(top_accumulator, row_exp_diff[row],
                             row_exp_diff[row + 2]);
  scale_accumulator_two_rows(bottom_accumulator, row_exp_diff[kPanelM + row],
...
#pragma unroll
  for (int k_offset = 0; k_offset < kSoftmaxPanelN; k_offset += kPanelK) {
    ...
    nvcuda::wmma::load_matrix_sync(b_fragment,
                                   value_panel + k_offset * kD + d_offset, kD);
    nvcuda::wmma::mma_sync(accumulator, a_fragment, b_fragment, accumulator);
  }
...
#pragma unroll 1
  for (int row_step = 0; row_step < 4; ++row_step) {
    ...
    const float exp_diff = make_probability_row(
        (top ? shared->score_top : shared->score_bottom) + local_row * kBlockN,
...
__device__ __forceinline__ uint32_t protocol_error_code(uint32_t phase,
                                                        int generation,
                                                        int warp) {
  return phase | ((static_cast<uint32_t>(generation) & 0xffffu) << 8) |
         (static_cast<uint32_t>(warp) & 0xffu);
}

__device__ __forceinline__ bool wait_for_counter(int* counter, int expected,
                                                 uint32_t* protocol_error,
                                                 uint32_t phase, int generation,
                                                 int warp) {
  const int lane = threadIdx.x & 31;
  int observed = 0;
#pragma unroll 1
...
extern "C" __global__ __launch_bounds__(
    kBaselineThreads,
    2) void sm70_native_bm32_d128_baseline(const __half* __restrict__ query,
  ...
  __half* output_group = output +
                         static_cast<int64_t>(group) * kOutputElements +
                         m_panel * kPanelM * kD;
  ...
  for (int index = thread; index < kPanelM * kD; index += kBaselineThreads) {
  ...
  for (int block_n = 0; block_n < nblocks; ++block_n) {
    if (warp < kQKWarps) {
      ...
      qk_single_accumulate(shared.query,
                           key_group + block_n * kBlockN * kD + n_offset * kD,
                           accumulator);
      nvcuda::wmma::store_matrix_sync(shared.score + n_offset, accumulator,
                                      kBlockN, nvcuda::wmma::mem_row_major);
    }
    __syncthreads();
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_d128_candidate(
  ...
  const int warp = thread >> 5;
  const __half* query_group = query + static_cast<int64_t>(group) * kQElements;
  stage_swizzled_q_panel(query_group, shared.query, thread, kCandidateThreads);
  ...
  if (warp < kQKWarps) {
    ...
    for (int block_n = 0; block_n < nblocks; ++block_n) {
      ...
      if (!wait_for_counter(&shared.score_ready, kQKWarps * (block_n + 1),
                            protocol_error, kProtocolTimeoutScore, block_n,
                            warp)) {
        return;
      }
...
#pragma unroll 1
      for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
        ...
        if (generation != 0) {
          scale_candidate_accumulators(top_accumulator, bottom_accumulator,
    ...
    store_accumulator_output(top_accumulator, output_group, shared.row_sum, 0,
                             d_offset);
...
void report_protocol_timeout(const ProtocolDiagnostics& diagnostics) {
  ...
  std::cerr << "Candidate counter protocol timeout: ctas="
            << diagnostics.timeout_ctas
            << ", first_cta=" << diagnostics.first_cta << ", code=0x"
            << std::hex << diagnostics.first_code << std::dec << '\n';
}
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"2 CTA/group; BM16, D256, 512 threads; "
               "serial QK-softmax-PV\",\n";
  std::cout << "    \"candidate\": \"2 CTA/group; each BM32, D128, 512 "
               "threads; 8 QK producer and 8 PV consumer warps\",\n";
  std::cout << "    \"candidate_output_halves\": \"CTA 0 writes D[0:128], CTA "
               "1 writes D[128:256]\",\n";
  std::cout << "    \"candidate_qk_order\": \"one K fragment, top then bottom "
               "M16\",\n";
  std::cout << "    \"candidate_pv_order\": \"one V fragment, top then bottom "
               "M16 for one D16\",\n";
  std::cout << "    \"candidate_pv_accumulators_per_warp\": 2,\n";
  std::cout << "    \"candidate_sync\": \"one initialization CTA barrier; "
               "warp-local score/P epochs afterwards\"\n";
  std::cout << "  },\n";
  std::cout << "  \"exactness\": {\"word_dtype\": \"uint32 packed fp16\", "
               "\"word_count\": "
  ...
  std::cout
      << "  \"timing\": {\"unit\": \"us per grid launch\", \"baseline\": ";
  ...
  std::cout
      << "  \"measurement\": {\"warmup_pairs\": " << args.warmup
      << ", \"rounds\": " << args.rounds
...
int run(const Args& args) {
  ...
  CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&device_candidate),
                        output_elements * sizeof(__half)));
  CUDA_CHECK(
      cudaMalloc(reinterpret_cast<void**>(&device_protocol_error),
                 static_cast<size_t>(candidate_ctas) * sizeof(uint32_t)));
  ...
  CUDA_CHECK(cudaMemcpy(device_value, host_value.data(),
                        kv_elements * sizeof(__half), cudaMemcpyHostToDevice));
  CUDA_CHECK(
      cudaMemset(device_protocol_error, 0,
                 static_cast<size_t>(candidate_ctas) * sizeof(uint32_t)));
