...
std::vector<uint32_t> capability_values(
  ...
  size_t bytes = 0;
  check_cublas(cublasLtMatmulAlgoCapGetAttribute(
                   &algo, attribute, nullptr, 0, &bytes),
...
bool set_algorithm_attribute(cublasLtMatmulAlgo_t* algo,
                             cublasLtMatmulAlgoConfigAttributes_t attribute,
                             T value) {
  return cublasLtMatmulAlgoConfigSetAttribute(algo, attribute, &value,
                                               sizeof(value)) ==
 ...
 public:
  LtRunner(int64_t m, int64_t n, int64_t k, int64_t workspace_bytes,
    ...
    check_cublas(cublasLtMatmulDescSetAttribute(
                     operation_, CUBLASLT_MATMUL_DESC_TRANSA, &transpose_a,
                     sizeof(transpose_a)),
                 "set transpose A");
    check_cublas(cublasLtMatmulDescSetAttribute(
                     operation_, CUBLASLT_MATMUL_DESC_TRANSB, &transpose_b,
                     sizeof(transpose_b)),
                 "set transpose B");
  ...
  py::list algorithm_info() const {
    ...
    for (size_t index = 0; index < algorithms_.size(); ++index) {
      ...
      item["id"] = algo_attribute<int32_t>(result.algo,
                                           CUBLASLT_ALGO_CONFIG_ID);
      item["tile"] = algo_attribute<uint32_t>(
          result.algo, CUBLASLT_ALGO_CONFIG_TILE_ID);
      item["split_k"] = algo_attribute<int32_t>(
      ...
      item["stages"] = algo_attribute<uint32_t>(
  ...
  void run(int64_t algorithm_index, torch::Tensor output,
           torch::Tensor input, torch::Tensor weight,
           torch::Tensor workspace) const {
    ...
    TORCH_CHECK(workspace.numel() >=
                    static_cast<int64_t>(algorithm.workspaceSize),
    ...
    check_cublas(
        cublasLtMatmul(
            at::cuda::getCurrentCUDABlasLtHandle(), operation_, &alpha_,
            input.data_ptr(), a_layout_, weight.data_ptr(), b_layout_, &beta_,
            output.data_ptr(), c_layout_, output.data_ptr(), c_layout_,
            &algorithm.algo, workspace_pointer, algorithm.workspaceSize,
  ...
  void query_heuristic_algorithms(int requested_algorithms) {
    ...
    try {
      ...
      check_cublas(cublasLtMatmulAlgoGetHeuristic(
                       at::cuda::getCurrentCUDABlasLtHandle(), operation_,
                       a_layout_, b_layout_, c_layout_, c_layout_, preference,
                       requested_algorithms, algorithms_.data(),
                       &returned_algorithms),
  ...
  void enumerate_algorithms() {
    ...
    for (int algorithm_id : algorithm_ids) {
      ...
      if (cublasLtMatmulAlgoInit(
              handle, CUBLAS_COMPUTE_32F, CUDA_R_32F, CUDA_R_16F,
              CUDA_R_16F, CUDA_R_16F, CUDA_R_16F, algorithm_id,
      ...
      const auto split_k_support = capability_attribute<int32_t>(
          base, CUBLASLT_ALGO_CAP_SPLITK_SUPPORT);
        ...
        for (uint32_t stage : stages) {
          for (int32_t custom = 0; custom <= custom_max; ++custom) {
            for (uint32_t swizzle = 0; swizzle <= swizzle_support; ++swizzle) {
              ...
              if (!set_algorithm_attribute(&configured,
              ...
              for (int32_t split_k : split_k_values) {
                for (uint32_t reduction : reduction_values) {
                  ...
                  if (!set_algorithm_attribute(
                          &split_configured, CUBLASLT_ALGO_CONFIG_SPLITK_NUM,
                          split_k) ||
