...
static_assert(kPPanelElements ==
              kProbabilityGroups * kPanelM * kProbabilityGroupN);
...
__device__ __forceinline__ void load_group_padded_probability_matrix_a_fragment(
  ...
  uint32_t address = static_cast<uint32_t>(
      __cvta_generic_to_shared(probability + first_group * 136 + slot * 8));
  uint32_t* words = reinterpret_cast<uint32_t*>(&fragment);
  asm volatile("ld.shared.v4.u32 {%0, %1, %2, %3}, [%4];"
               : "=r"(words[0]), "=r"(words[1]), "=r"(words[2]), "=r"(words[3])
               : "r"(address)
...
#pragma unroll
  ...
  if (lane < kSoftmaxPanelN / 4) {
    ...
    __half2* first_pair = reinterpret_cast<__half2*>(
        probability +
        group_padded_probability_matrix_a_offset(probability_row, column));
    __half2* second_pair = reinterpret_cast<__half2*>(
        probability +
        group_padded_probability_matrix_a_offset(probability_row, column + 2));
    *first_pair = __float22half2_rn(make_float2(values.x, values.y));
    *second_pair = __float22half2_rn(make_float2(values.z, values.w));
  }
...
__device__ __forceinline__ void update_group_padded_probability_pv_panel(
    ...
    AccumulatorFragment& accumulator_bottom) {
  {
    ...
    PVMatrixBFragment b_fragment;
    nvcuda::wmma::load_matrix_sync(b_fragment, value_panel + d_offset, kD);
    load_group_padded_probability_matrix_a_fragment(a_fragment, probability_top,
                                                    0);
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_p_group_pad_baseline(
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_p_group_pad_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"probability_layout\": {"
               ...
               "\"baseline_offset\": \"group*128 + swizzled_slot(row)*8 + "
               "inner; group=column/8\", "
               "\"candidate_offset\": \"group*136 + swizzled_slot(row)*8 + "
               "inner; group=column/8\", "
               "\"bank_proof\": \"group base is 136 fp16 = 272B = 68 banks, or "
               "+4 mod 32; 32-bit store bank=(4*swizzled_slot+4*group+inner/2) "
               "mod 32, so groups 0..3 use four distinct banks and reduce the "
               "static four-way collision to one-way\", "
               "\"pv_matrix_a_loader\": \"P-only; two ld.shared.v4.u32 "
               "(LDS.128) per M16xK16 fragment; second address is first +272B "
               "with no second generic offset/cvta; Q loader unchanged\"},\n";
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"1 CTA/group; BM32, 512 threads; accepted "
               "all-P pair-scratch QK, softmax, PV\",\n";
  std::cout
      << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same accepted "
         "pair-scratch path with P 136-half group-pad layout only\",\n";
  std::cout
      << "    \"qk\": \"warps 0..7; one K fragment then top/bottom M16\",\n";
  std::cout << "    \"softmax\": \"16 warps; matching top/bottom rows with "
               "32-lane reductions\",\n";
  std::cout << "    \"pv\": \"16 warps; one D16 and two FP32 accumulators, one "
               "V fragment then top/bottom M16\",\n";
  ...
  std::cout << "},\n";
  std::cout
      << "  \"resource_gate\": {\"max_registers_per_thread\": 64, "
      ...
      << ", \"min_active_ctas_per_sm\": 2, "
         "\"runtime_pass\": "
      << (resource_pass ? "true" : "false")
      << ", \"ptxas_validation\": \"compile with -O3 and --ptxas-options=-v; "
  ...
  std::cout << ",\n";
  std::cout
      << "  \"tradeoff\": {\"candidate_extra_shared_bytes\": 384, "
  ...
  if (timing_available) {
    ...
    std::cout
        << "null, \"net_candidate_speedup_pct_after_overlap_removal\": null, "
  ...
  std::cout << "  \"measurement\": {\"warmup_pairs\": " << args.warmup
            ...
            << ", \"interleaving\": \"baseline/candidate order alternates each "
               "round\", "
               "\"formal_gate\": \"100 paired rounds x 8 launches; at least 95 "
               "candidate wins and at least 2 percent wall-time win\"}\n";
  std::cout << "}\n";
}
