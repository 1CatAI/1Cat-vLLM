...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_pv_load_fence_baseline(
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_pv_load_fence_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout
      << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same accepted "
         "pair-scratch path with a second-K16 PV compiler fence only\",\n";
  std::cout
