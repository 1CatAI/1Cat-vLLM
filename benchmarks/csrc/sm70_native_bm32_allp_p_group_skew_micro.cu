...
#pragma unroll
  ...
  if (lane < kSoftmaxPanelN / 4) {
    ...
    __half2* first_pair = reinterpret_cast<__half2*>(
        probability +
        group_skewed_probability_matrix_a_offset(probability_row, column));
    __half2* second_pair = reinterpret_cast<__half2*>(
        probability +
        group_skewed_probability_matrix_a_offset(probability_row, column + 2));
    *first_pair = __float22half2_rn(make_float2(values.x, values.y));
    *second_pair = __float22half2_rn(make_float2(values.z, values.w));
  }
...
__device__ __forceinline__ void update_group_skewed_probability_pv_panel(
    ...
    AccumulatorFragment& accumulator_bottom) {
  {
    ...
    PVMatrixBFragment b_fragment;
    nvcuda::wmma::load_matrix_sync(b_fragment, value_panel + d_offset, kD);
    load_group_skewed_probability_matrix_a_fragment(a_fragment, probability_top,
                                                    0);
    nvcuda::wmma::mma_sync(accumulator_top, a_fragment, b_fragment,
                           accumulator_top);
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_p_group_skew_baseline(
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_allp_p_group_skew_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"probability_layout\": {"
               ...
               "\"candidate_offset\": \"group*128 + "
               "((swizzled_slot(row)+group)&15)*8 + inner; group=column/8\", "
               "\"bank_proof\": \"32-bit store "
               "bank=(4*swizzled_slot+4*group+inner/2) mod 32; groups 0..3 map "
               "to distinct banks, reducing the static four-way same-bank "
               "group collision to one-way\", "
               "\"pv_matrix_a_loader\": \"P-only; two ld.shared.v4.u32 "
               "(LDS.128) per M16xK16 fragment; Q loader unchanged\"},\n";
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"1 CTA/group; BM32, 512 threads; accepted "
               "all-P pair-scratch QK, softmax, PV\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; same "
               "accepted pair-scratch path with P group-skew layout only\",\n";
  std::cout
  ...
  std::cout << ",\n";
  std::cout
      << "  \"tradeoff\": {\"qk_k_fragment_reuse\": 2, "
