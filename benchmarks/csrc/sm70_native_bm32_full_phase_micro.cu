...
extern "C" __global__
__launch_bounds__(kBaselineThreads, 2) void sm70_native_bm32_full_phase_baseline(
...
extern "C" __global__
__launch_bounds__(kCandidateThreads, 2) void sm70_native_bm32_full_phase_candidate(
    const __half* __restrict__ query, const __half* __restrict__ key,
    const __half* __restrict__ value, __half* __restrict__ output, int groups,
    int nblocks) {
...
#pragma unroll
    for (int panel = 0; panel < kSoftmaxPanelsPerBlock; ++panel) {
      ...
      const float bottom_exp_diff = make_probability_row(
          ...
          shared.probability_bottom, phase_warp, shared.row_max, shared.row_sum,
          kPanelM + phase_warp, panel);
      if ((threadIdx.x & 31) == 0) {
...
void print_json(const Args& args, const cudaDeviceProp& properties,
  ...
  std::cout << "  \"paths\": {\n";
  std::cout << "    \"baseline\": \"2 CTA/group; BM16, 512 threads; serial "
               "QK-softmax-PV with shared output\",\n";
  std::cout << "    \"candidate\": \"1 CTA/group; BM32, 512 threads; "
               "phase-reuse QK, softmax, PV\",\n";
  std::cout << "    \"candidate_qk\": \"warps 0..7; one K fragment then "
               "top/bottom M16\",\n";
  std::cout << "    \"candidate_softmax\": \"16 warps; each handles matching "
               "top/bottom rows with 32-lane reductions\",\n";
  std::cout << "    \"candidate_pv\": \"16 warps; one D16 and two FP32 "
               "accumulators, one V fragment then top/bottom M16\",\n";
...
int run(const Args& args) {
  ...
  CUDA_CHECK(cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount,
                                    args.device));
  const KernelResources baseline_resources =
      query_resources(sm70_native_bm32_full_phase_baseline, kBaselineThreads);
  const KernelResources candidate_resources =
      query_resources(sm70_native_bm32_full_phase_candidate, kCandidateThreads);
