...
struct alignas(256) XQATCSmem256WideLayout {
  ...
  static constexpr int kKVStride =
      PADDED_SMEM ? (ALIGNED_PADDED_SMEM
                         ? kXQATC256WideAlignedPaddedKVStride
...
struct alignas(256) XQATCStagedPVSmem256Wide {
  static constexpr int kKVStride = PADDED_SMEM ? kXQATC256WidePaddedKVStride
                                                 : kXQATCStride;
...
bool xqa_g6_dual_cta_dense_enabled() {
  const char* value =
      std::getenv("VLLM_FLASH_V100_XQA_G6_DUAL_CTA_DENSE");
  return value != nullptr && value[0] == '1';
}
...
bool xqa_block16_layout_required() {
  const char* value =
      std::getenv("VLLM_FLASH_V100_XQA_BLOCK16_LAYOUT_REQUIRE");
  return value != nullptr && value[0] == '1';
}
...
bool xqa_block16_layout_trace_enabled() {
  static const bool enabled = [] {
    const char* value =
        std::getenv("VLLM_FLASH_V100_XQA_BLOCK16_LAYOUT_TRACE");
    return value != nullptr && value[0] == '1';
  }();
  return enabled;
}
...
bool xqa_block784_index_trace_enabled() {
  static const bool enabled = [] {
    const char* value =
        std::getenv("VLLM_FLASH_V100_XQA_BLOCK784_INDEX_TRACE");
    return value != nullptr && value[0] == '1';
  }();
  return enabled;
}
...
bool xqa_aligned_padded_smem_enabled() {
  const char* value =
      std::getenv("VLLM_FLASH_V100_XQA_ALIGNED_PADDED_SMEM");
  return value != nullptr && value[0] == '1';
}
...
__device__ __forceinline__ uint32_t fp8_e5m2_pair_to_half2_bits(
    const uint16_t raw_pair) {
  return (static_cast<uint32_t>(raw_pair & 0x00ffu) << 8) |
         (static_cast<uint32_t>(raw_pair & 0xff00u) << 16);
}

__device__ __forceinline__ uint4 fp8_e5m2_vector_to_half8(
    const uint64_t raw) {
  return make_uint4(
...
__device__ __forceinline__ uint4 load_xqa_tc_kv_vector(
    ...
    const int copy_idx,
    const int panel_d_stride_uint4,
...
__device__ __forceinline__ void load_xqa_tc_kv_panel(
    __half* __restrict__ shared_kv,
    const void* __restrict__ kv_cache,
    const int* __restrict__ page_ids,
    const int valid_kv_tile_rows,
    const int panel_d_stride_uint4,
    const int kv_smem_stride_uint4,
    const int tile_page_offset,
    const int kv_tile_start,
    const int block_size,
    const int kv_head_idx,
...
template<int PARTITION_SIZE, int GROUP_SIZE, bool PADDED_SMEM,
         int NUM_THREADS, int MIN_BLOCKS_PER_SM, int BLOCK_SIZE,
         bool CONTIGUOUS_HKV1_LAYOUT, bool ALIGNED_PADDED_SMEM,
         int KV_DTYPE>
        ...
        if (warp_id < (kXQATCBlockN / WMMA_N)) {
          ...
          for (int k_tile = 0; k_tile < (kPanelDim / WMMA_K); ++k_tile) {
            ...
            volta::load_matrix_sync(
                qk_b_frag,
                sK + tile_n * SmemLayout::kKVStride + k_offset,
    ...
    for (int panel_idx = 0; panel_idx < kNumPanels; ++panel_idx) {
      ...
      for (int kv_tile_start = 0; kv_tile_start < valid_k_rows;
        ...
        if (tid < GROUP_SIZE * kXQATC256WideThreadsPerRow) {
          const int row = tid / kXQATC256WideThreadsPerRow;
          const __half* sP_row =
              sP + row * kXQATCBlockN + kv_tile_start;
          #pragma unroll
          for (int token = 0; token < kXQATCBlockN; ++token) {
            ...
            for (int d_iter = 0; d_iter < (kPanelDim / kWarpSize); ++d_iter) {
              ...
              const int acc_idx =
                  panel_idx * (kPanelDim / kWarpSize) + d_iter;
              out_acc[acc_idx] =
  ...
  if (tid < GROUP_SIZE * kXQATC256WideThreadsPerRow) {
    ...
    for (int d = thread_in_row; d < D; d += kXQATC256WideThreadsPerRow) {
      const float output_scale =
          KV_DTYPE == flash_v100::KV_CACHE_DTYPE_FP16 ? inv_row_sum
                                                       : inv_row_sum * v_scale;
...
template<int PARTITION_SIZE, int GROUP_SIZE, bool PADDED_SMEM,
         int NUM_THREADS, int MIN_BLOCKS_PER_SM>
__global__ void __launch_bounds__(NUM_THREADS, MIN_BLOCKS_PER_SM)
flash_attention_decode_xqa_tc_qk_softmax_staged_kernel_256_wide(
    ...
    float* __restrict__ max_logits,
    float* __restrict__ exp_sums,
    float* __restrict__ online_rescales,
    const int* __restrict__ block_table,
    const int* __restrict__ seq_lens,
    const int* __restrict__ active_num_partitions,
    const int batch_size,
    ...
    const int64_t q_stride0,
    const int64_t q_stride1,
    const int64_t probability_stride0,
    const int64_t probability_stride1,
    const int64_t probability_stride2,
    const int64_t stats_stride0,
    const int64_t stats_stride1,
    const int64_t online_rescale_stride0,
    const int64_t online_rescale_stride1,
    const int64_t k_block_stride,
    const int64_t k_token_stride,
    const int64_t k_head_stride,
    const float softmax_scale) {
  ...
  for (int block_n = 0; block_n < num_k_tiles; ++block_n) {
    ...
    for (int idx = tid; idx < GROUP_SIZE * valid_k_rows; idx += NUM_THREADS) {
      ...
      __half* probability_ptr =
          probabilities + static_cast<int64_t>(batch_idx) * probability_stride0 +
  ...
  if (tid < GROUP_SIZE * kXQATC256WideThreadsPerRow) {
    ...
    if (thread_in_row == 0) {
      ...
      online_rescales[static_cast<int64_t>(batch_idx) *
...
flash_attention_decode_xqa_tc_pv_staged_kernel_256_wide(
      ...
      for (int v_tile_start = 0; v_tile_start < valid_k_rows;
        ...
        if (tid < GROUP_SIZE * kXQATC256WideThreadsPerRow) {
          ...
          const __half* probability_ptr =
              probabilities + static_cast<int64_t>(batch_idx) * tmp_out_stride0 +
              static_cast<int64_t>(head_idx) * tmp_out_stride1 +
              static_cast<int64_t>(partition_idx) * tmp_out_stride2 +
  ...
  if (tid < GROUP_SIZE * kXQATC256WideThreadsPerRow) {
    ...
    __half* tmp_out_ptr =
        ...
        static_cast<int64_t>(head_idx) * tmp_out_stride1 +
        static_cast<int64_t>(partition_idx) * tmp_out_stride2;
    for (int d = thread_in_row; d < D;
         d += kXQATC256WideThreadsPerRow) {
      tmp_out_ptr[d] =
          __float2half(out_acc[d / kWarpSize] * inv_row_sum);
    }
  }
}
...
template<int PARTITION_SIZE, int GROUP_SIZE, bool PADDED_SMEM,
         int NUM_THREADS = kXQATC256WideThreads,
...
void launch_flash_attention_decode_paged_xqa_tc_256_wide(
    ...
    const int split_reduce_dim_tile,
    cudaStream_t stream) {
  const int batch_size = q.size(0);
  const int num_heads_q = q.size(1);
  const int num_heads_kv = k_cache.size(2);
    ...
    do {                                                                       \
      ...
      flash_attention_decode_xqa_tc_partition_kernel_256_wide<                 \
          ...
          <<<partition_grid, NUM_THREADS, shared_mem, stream>>>(               \
              ...
              active_num_partitions.data_ptr<int>(), batch_size,               \
              max_num_blocks, launch_num_partitions, num_heads_q,              \
...
void launch_flash_attention_decode_paged_xqa_tc_256_staged(
  ...
  flash_attention_decode_xqa_tc_qk_softmax_staged_kernel_256_wide<
      ...
      <<<partition_grid, kQKThreads, qk_shared_mem, stream>>>(
          ...
          max_logits.data_ptr<float>(),
          exp_sums.data_ptr<float>(),
          online_rescales.data_ptr<float>(),
          block_table.data_ptr<int>(),
          seq_lens.data_ptr<int>(),
          active_num_partitions.data_ptr<int>(),
          batch_size,
          ...
          max_logits.stride(0),
          max_logits.stride(1),
          online_rescales.stride(0),
          online_rescales.stride(1),
          k_cache.stride(0),
          k_cache.stride(1),
          k_cache.stride(2),
          softmax_scale);
...
at::Tensor flash_attention_decode_paged_xqa(
  ...
  const bool use_g6_dual_cta_dense =
      !use_padded_smem && k_cache.size(1) == 784 &&
  ...
  const bool supports_block16_index =
      use_g6_dual_cta && k_cache.size(1) == 16;
  ...
  const bool use_block784_index =
      use_g6_dual_cta && k_cache.size(1) == 784 &&
      xqa_block784_index_enabled();
  const bool use_aligned_padded_smem =
      use_padded_smem && use_block784_index &&
  ...
  const int block16_layout_mode =
      requested_block16_layout_mode == 1 && supports_block16_index
          ? 1
          : requested_block16_layout_mode == 2 &&
                    supports_block16_contiguous_layout
                ? 2
                : 0;
  ...
  if (xqa_block784_index_trace_enabled() && use_block784_index &&
    ...
    TORCH_WARN("Flash-V100 XQA block784 index specialization active for KV "
               "shape [blocks,", k_cache.size(1), ",", k_cache.size(2), ",",
               k_cache.size(3), "]");
    ...
    if (use_block784_index) {
      if (use_aligned_padded_smem) {
        launch_flash_attention_decode_paged_xqa_tc_256_wide<
            ...
            q, k_cache, v_cache, out, block_table, seq_lens, tmp_out,
            max_logits, exp_sums, active_num_partitions, softmax_scale,
            k_scale, v_scale, launch_num_partitions, use_split_reduce,
            split_reduce_dim_tile, stream);
