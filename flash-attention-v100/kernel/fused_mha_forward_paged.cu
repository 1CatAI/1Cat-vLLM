...
// head_dim 512 (gemma global layers): small blocks so 512-wide tiles fit smem.
...
flash_attention_forward_kernel_paged(
    ...
    // earliest query in this row-block are fully masked -> skip those tiles.
    int first_n_tile = 0;
    if constexpr (IS_CAUSAL) {
        if (window >= 0) {
...
at::Tensor flash_attention_prefill_paged(
    ...
    TORCH_CHECK((D <= 256 || D == 512) && D % 8 == 0 && D % 2 == 0,
                "D must be even, multiple of 8, and <=256 or ==512");
