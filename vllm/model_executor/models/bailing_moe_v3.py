...
Ported for 1Cat-vLLM (SM70/V100): replaced upstream FusedMoEFactory with
1Cat's FusedMoE(PluggableLayer) and adapted import paths to 1Cat's layout.
"""
...
class BailingMoeV3MLP(nn.Module):
    def __init__(
        ...
        self.act_fn = SwigluStepAndMul(swiglu_limit=swiglu_limit)
...
class BailingMoeV3KimiDeltaAttention(PluggableLayer, MambaBase):
    """Kimi Delta Attention (KDA) layer for BailingMoeV3.

    Uses GDN (Gated Delta Net) attention with a custom CUDA kernel
    (bailing_v3_kda_attention) for the recurrent KDA step.
    """
...
class BailingMoeV3Gate(nn.Module):
    def __init__(
        self, config: PretrainedConfig, prefix: str = "gate"
    ) -> None:
        super().__init__()
        self.expert_bias = (
            nn.Parameter(torch.empty(config.num_experts, dtype=torch.float32))
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return self.gate(hidden_states.to(torch.float32))
...
class BailingMoeV3MoE(nn.Module):
    def __init__(
        ...
        # 1Cat-vLLM port: replaced FusedMoEFactory(...) with FusedMoE(...)
        # 1Cat's FusedMoE is a PluggableLayer that owns its own MoERunner;
        # pass the full intermediate_size (it handles TP sharding internally).
...
class BailingMoeV3ForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for orig_name, weight in weights:
            ...
            # 1Cat-vLLM port: also handle shared_experts weights that live inside
            # FusedMoE.runner._shared_experts. The checkpoint names use
            # "mlp.shared_experts." while 1Cat stores them under
            # "mlp.experts.runner._shared_experts.".
            if ".mlp.shared_experts." in name:
                runner_name = name.replace(
                    ".mlp.shared_experts.", ".mlp.experts.runner._shared_experts."
                )
                if load_param(runner_name, weight):
                    continue
