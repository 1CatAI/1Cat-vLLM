...
def gather_initial_states(
    ...
    """Gather state rows and zero rows without an initialized state."""
    assert state.ndim >= 2 and state.is_cuda
    assert indices.ndim == 1 and has_initial_state.ndim == 1
    assert indices.shape == has_initial_state.shape
    assert indices.device == state.device == has_initial_state.device
