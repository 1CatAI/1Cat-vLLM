...
class Glm5NextVideoProcessor(BaseVideoProcessor):
    ...
    def get_number_of_video_patches(
        ...
        """Number of temporal/spatial patches for a raw video shape."""
        ...
        patch_expand_factor = videos_kwargs.get(
            "patch_expand_factor", self.patch_expand_factor
        )
        min_pixels, max_pixels = _pixel_budget(
            videos_kwargs.get("min_image_tokens", self.min_image_tokens),
            videos_kwargs.get("max_image_tokens", self.max_image_tokens),
            patch_size,
            merge_size,
            temporal_patch_size,
        )
        resized_height, resized_width = smart_resize(
            t=max(num_frames, temporal_patch_size),
            h=height,
            w=width,
            t_factor=temporal_patch_size,
            h_factor=patch_size * merge_size * patch_expand_factor,
...
class Glm5NextProcessor(ProcessorMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        processor_config_path = cached_file(
            model_path,
