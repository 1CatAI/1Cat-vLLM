...
def _qsa_pre_indexer_kernel(
    ...
    if pid < num_k_work:
        ...
        if work_in_request < num_groups:
            ...
            # Match the unfused path's storage-dtype pooled tensor before
            # RMSNorm. V100 executes this route in FP16.
            pooled = tl.sum(source, axis=0) / COMPRESS_RATIO
            if CACHE_IS_FP16:
...
def qsa_pre_indexer(
    ...
    *,
    ...
    assert q.stride(-1) == 1
    assert k.stride(-1) == 1
    assert q.dtype == k.dtype == q_out.dtype
    assert q.dtype == state_cache.dtype == compressed_cache.dtype
    ...
    _qsa_pre_indexer_kernel[(num_k_work + num_q_work,)](
        ...
        CACHE_IS_FP16=q.dtype == torch.float16,
        MROPE_H=section[1],
        MROPE_W=section[2],
        num_warps=1,
    )
