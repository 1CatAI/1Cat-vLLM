...
@torch.compile(**_INDEXER_COMPILE)
def _fused_indexer_weight_only_scale(
...
class Glm5NextTailCache(DeepseekV32IndexerCache):
    ...
    Stores raw model-dtype K (``head_dim``) as the "K" half of each block and
    gate score (``head_dim``) as the "V" half -- not the fp8-compressed
...
class Indexer(nn.Module):
    def __init__(
        ...
        self.index_kpool_compress_gate = nn.Parameter(
            torch.empty(self.head_dim, hidden_size, dtype=self.activation_dtype)
        )
    ...
    def forward(
        ...
        sm70_fp16_indexer = current_platform.is_cuda() and (
            current_platform.is_device_capability((7, 0))
        )
        if sm70_fp16_indexer:
            assert q.dtype == torch.float16
            q_index = fwht128(q).view(-1, self.n_head, self.head_dim)
            weights = _fused_indexer_weight_only_scale(
                weights, self.softmax_scale * self.n_head**-0.5
            )
        else:
            q_fp8, q_scale = fwht128_quant_fp8(q)
            q_index = q_fp8.view(-1, self.n_head, self.head_dim)
        ...
        if self.n_head < 32:
            pad = 32 - self.n_head
            q_index = _pad_indexer_heads(q_index, pad)
