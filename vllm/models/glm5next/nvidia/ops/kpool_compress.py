...
def _fwht_quant_kernel(
    ...
    # Match the model activation dtype's materialization before quantizing.
    if MATERIALIZE_FP16:
...
def fwht128_quant_fp8(q: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    Args:
        q: ``[rows, 128]`` FP16 or BF16 — one head vector per row.
    ...
    n_rows = q.shape[0]
    software_fp8 = _is_exact_sm70_cuda()
    ...
    qout = q_fp8.view(torch.uint8) if software_fp8 else q_fp8
    _fwht_quant_kernel[grid](
        ...
        MATERIALIZE_FP16=q.dtype == torch.float16,
        num_warps=2,
    )
    return q_fp8, q_scale
...
@triton.jit
def _fwht_kernel(q_ptr, out_ptr, n_rows, BLOCK_R: tl.constexpr):
    pid = tl.program_id(0)
    rows = pid * BLOCK_R + tl.arange(0, BLOCK_R)
    rmask = rows < n_rows
    offs = tl.arange(0, 128)
    x = tl.load(
        q_ptr + rows[:, None] * 128 + offs[None, :],
        mask=rmask[:, None],
        other=0.0,
    ).to(tl.float32)
    ...
    x = _fwht_stage(x, n, BLOCK_R * 2, 32)
    x = _fwht_stage(x, n, BLOCK_R, 64)
    x = tl.reshape(x * 0.08838834764831845, (BLOCK_R, 128))
    tl.store(
        out_ptr + rows[:, None] * 128 + offs[None, :],
        x,
        mask=rmask[:, None],
    )


def fwht128(q: torch.Tensor) -> torch.Tensor:
    """Rotate contiguous 128-wide FP16/BF16 rows without quantizing them."""
    assert q.ndim == 2 and q.shape[1] == 128, q.shape
    ...
    if q.shape[0] == 0:
        return out
    block_r = 32
    _fwht_kernel[(triton.cdiv(q.shape[0], block_r),)](
        q,
        out,
        q.shape[0],
        BLOCK_R=block_r,
...
def _kpool_softmax_rotate_write_cache_kernel(
    ...
    x = acc / denom
    x = tl.where(do_write, x, 0.0)
    if MATERIALIZE_FP16:
        x = x.to(tl.float16).to(tl.float32)
    else:
        x = x.to(tl.bfloat16).to(tl.float32)

    # Match the unfused pooled-K path's activation precision before quantization.
    x = _hadamard128(x)
    if MATERIALIZE_FP16:
        x = x.to(tl.float16).to(tl.float32)
    else:
    ...
    if SOFTWARE_FP8:
        quantized = fp32_to_fp8_e4m3fn_bits(quantized)
...
def kpool_compress_and_write_cache(
    ...
    Args:
        ...
        slot_k: ``[n_pools, pool_size, head_dim]`` FP16/BF16 raw per-token K.
    ...
    assert slot_k.dtype in (torch.bfloat16, torch.float16)
    assert slot_score.dtype == slot_k.dtype
    ...
    software_fp8 = _is_exact_sm70_cuda()
    buf_fp8 = buf if software_fp8 else buf.view(torch.float8_e4m3fn)
    ...
    compressed_k_ptr = (
        ...
        if software_fp8 and compressed_k.dtype == torch.float8_e4m3fn
        else compressed_k
    )
    ...
    _kpool_softmax_rotate_write_cache_kernel[(slot_k.shape[0],)](
        ...
        SOFTWARE_FP8=software_fp8,
        MATERIALIZE_FP16=slot_k.dtype == torch.float16,
...
def kpool_seed_tail_cache(
    ...
    assert tail_kv_cache.dtype in (torch.bfloat16, torch.float16)
    assert key.dtype == tail_kv_cache.dtype
    assert gate_score.dtype == key.dtype
    n = tslot.shape[0]
    if n == 0:
        return
...
def kpool_decode_update_and_maybe_write_cache_batched(
    ...
    Args:
        ...
        tail_kv_cache: paged tail cache ``[num_blocks, 2, pool_size, head_dim]``
            FP16/BF16 (K at half 0, gate score at half 1).
        tail_slot_mapping: ``[num_requests, next_n]`` int32.
        key / slot_score: ``[num_requests, next_n, head_dim]`` FP16/BF16.
    ...
    assert key.dtype in (torch.bfloat16, torch.float16)
    assert slot_score.dtype == key.dtype
    assert tail_kv_cache.dtype == key.dtype
    assert ape.dtype == torch.float32
    assert kv_cache.dtype == torch.uint8
    ...
    _kpool_decode_update_batched_kernel[(num_requests,)](
        ...
        ROUND_SCALE=round_scale,
        SOFTWARE_FP8=software_fp8,
        MATERIALIZE_FP16=key.dtype == torch.float16,
        BLOCK_D=triton.next_power_of_2(head_dim),
    )
